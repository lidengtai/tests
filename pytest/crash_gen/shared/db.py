...
class DbConnRest(DbConn):
    ...
    def _doSql(self, sql):
        ...
        try:
            r = requests.post(
                self._url, data=sql, auth=HTTPBasicAuth("root", "taosdata")
            )
        except:
        ...
        if Config.isSet("use_shadow_db"):
            if sql[:11] == "INSERT INTO":
                if sql[:16] == "INSERT INTO db_0":
                    ...
                    raise CrashGenError(
                        "Did not find db_0 in INSERT statement: {}".format(sql)
                    )
            ...
            if sql[:12] == "CREATE TABLE":
                if sql[:17] == "CREATE TABLE db_0":
                    ...
                    raise CrashGenError(
                        "Did not find db_0 in CREATE TABLE statement: {}".format(sql)
                    )
            else:  # not an insert statement
                pass
...
class DbConnNative(DbConn):
    ...
    def openByType(self):  # Open connection
        ...
        with cls._lock:  # force single threading for opening DB connections. # TODO: whaaat??!!!
            ...
            self._tdSql = MyTDSql(
                dbTarget.hostAddr, dbTarget.cfgPath
    ...
    def execute(self, sql):
        if not self.isOpen:
            ...
            raise CrashGenError(
                ...
                CrashGenError.DB_CONNECTION_NOT_OPEN,
            )
        ...
        cls = self.__class__
        cls.totalRequests += 1
        Logging.debug("[SQL] Execution Result, nRows = {}, SQL = {}".format(nRows, sql))
        return nRows
    ...
    def query(self, sql):  # return rows affected
        ...
        cls = self.__class__
        cls.totalRequests += 1
        Logging.debug("[SQL] Query Result, nRows = {}, SQL = {}".format(nRows, sql))
        return rows, nRows
...
class DbManager:
    ...
    def __init__(self, cType, dbTarget):
        ...
        self._dbConn = (
            DbConn.createNative(dbTarget)
            if (cType == "native")
            else DbConn.createRest(dbTarget)
        )
        try:
            ...
            if err.msg == "client disconnected":  # cannot open DB connection
                ...
                print(
                    "Failed to connect to DB, errno = {}, msg: {}".format(
                        Helper.convertErrno(err.errno), err.msg
                    )
                )
                raise
